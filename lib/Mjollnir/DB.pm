...
sub get_names {
    ...
    my $dbh = $self->dbh;
    my @names = map { @$_ } @{ $dbh->selectall_arrayref(
...
sub add_name {
    ...
    if ($row) {
        $dbh->do('UPDATE player_names SET steam_id = ?, name = ?, timestamp = ? WHERE id = ?', {}, $id, $name, time, $row->[0]);
    }
    else {
        $dbh->do('INSERT INTO player_names (steam_id, name, timestamp) VALUES (?, ?, ?)', {}, $id, $name, time);
    }
}
...
sub get_ips {
    ...
    my $dbh = $self->dbh;
    my @ips = map { @$_ } @{ $dbh->selectall_arrayref(
...
sub add_ip {
    ...
    if ($row) {
        $dbh->do('UPDATE player_ips SET timestamp = ? WHERE id = ?', {}, time, $row->[0]);
    }
    else {
        $dbh->do('INSERT INTO player_ips (steam_id, ip, timestamp) VALUES (?, ?, ?)', {}, $id, $ip, time);
    }
}
...
sub check_banned_id {
    ...
    my $dbh = $self->dbh;
    my ($match) = @{ $dbh->selectrow_arrayref(
...
sub check_banned_ip {
    ...
    my $dbh = $self->dbh;
    my ($match) = @{ $dbh->selectrow_arrayref(
        'SELECT COUNT(*) FROM ip_bans WHERE ip = ?',
        {},
        $ip,
    )};
    return $match;
...
sub add_ip_ban {
    ...
    if ($row) {
        $dbh->do('UPDATE ip_bans SET timestamp = ? WHERE id = ?', {}, time, $row->[0]);
    }
    else {
        $dbh->do('INSERT INTO ip_bans (steam_id, ip, timestamp) VALUES (?, ?, ?)', {}, $id, $ip, time);
    }
}
...
sub add_id_ban {
    ...
    if ($row) {
        $dbh->do('UPDATE id_bans SET timestamp = ? WHERE id = ?', {}, time, $row->[0]);
    }
    else {
        $dbh->do('INSERT INTO id_bans (steam_id, timestamp) VALUES (?, ?)', {}, $id, time);
    }
}
